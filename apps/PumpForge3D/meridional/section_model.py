...
from apps.PumpForge3D.meridional.bezier import sample_bezier
...
class MeridionalBezierParams:
    ...
    hub_p2_z_norm: float
    hub_p2_r_offset: float
    tip_p2_z_norm: float
    tip_p2_r_offset: float
...
def build_section(params: MeridionalBezierParams) -> MeridionalSection2D:
    ...
    hub_p2_z = params.hub_p2_z_norm * Lz
    hub_r_base = params.hub_r_in_m + (params.hub_r_out_m - params.hub_r_in_m) * (
        hub_p2_z / Lz
    )
    hub_p2 = np.array([hub_p2_z, hub_r_base + params.hub_p2_r_offset], dtype=float)
    ...
    tip_p2_z = params.tip_p2_z_norm * Lz
    tip_r_base = tip_r_in + (tip_r_out - tip_r_in) * (tip_p2_z / Lz)
    tip_p2 = np.array([tip_p2_z, tip_r_base + params.tip_p2_r_offset], dtype=float)
    tip_ctrl = np.vstack([tip_p0, tip_p1, tip_p2, tip_p3, tip_p4])

    le_r_mid = 0.5 * (params.hub_r_in_m + tip_r_in)
    le_ctrl_point = np.array([params.le_ctrl_z_offset_m, le_r_mid], dtype=float)
    le_ctrl = np.vstack([hub_p0, le_ctrl_point, tip_p0])

    te_r_mid = 0.5 * (params.hub_r_out_m + tip_r_out)
    te_ctrl_point = np.array([Lz + params.te_ctrl_z_offset_m, te_r_mid], dtype=float)
    te_ctrl = np.vstack([hub_p4, te_ctrl_point, tip_p4])
