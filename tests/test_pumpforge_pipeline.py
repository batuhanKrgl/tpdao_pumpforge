...
def test_pumpforge_pipeline(tmp_path: Path) -> None:
    ...
    pump1d_input = repo_root / "apps" / "PumpForge" / "io" / "examples" / "pump1d_input.example.json"
    pump3d_input = repo_root / "apps" / "PumpForge" / "io" / "examples" / "pump3d_input.example.json"
    ...
    pump3d_payload = merge_dicts(load_json(pump3d_input), {"from_pump1d": pump1d_result["export_for_3d"]})
    pump3d_input_path = tmp_path / "pump3d_input.json"
    save_json(pump3d_input_path, pump3d_payload)

    pump3d_output_path = tmp_path / "pump3d_output.json"
    pump3d_result = run_pump3d(pump3d_input_path, pump3d_output_path)
